...
if($num > 0) {
    ...
    for($i=0; $i<$n1; $i++) {
        $total_value= 0;
        $data['data'] .= ' 
        <div class="tbl_item_list active_request">';
        ...
        for($j=0; $j<$n2; $j++) {
            ...
            <div class="address_box">
                ...
                '.$main[$i][0]['district'].', '.$main[$i][0]['state'].' ('.$main[$i][0]['pincode'].')<br/>
            </div>
            <div class="action_box">';
            $aw= $main[$i][0]['assigned_worker'];
            $data['data'] .= '
                <span>Assigned to: '.$salon_data[$aw]['name'].'</span>
                <p>Total: &#8377;'.$total_value.'</p>
            </div>
            <div class="action_box">
                <a class="btn btn-success" href="request_handler.php?reference-id='.$main[$i][0]['booking_id'].'&assigned-to='.$aw.'&worker-name='.$salon_data[$aw]['name'].'">Open Service Page</a>
                <a class="wtsapp_share_link_btn" href="whatsapp://send?text=https://www.work.haazir.in/request_handler.php?reference-id='.$main[$i][0]['booking_id'].'&assigned-to='.$aw.'&worker-name='.$salon_data[$aw]['name'].'" data-action="share/whatsapp/share/" target="">
